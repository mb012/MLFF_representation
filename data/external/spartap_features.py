...
Updated 2024 to support newer versions of libraries
"""
import os
# import os.path
# import sys
...
import math
from Bio import PDB
from Bio.PDB import DSSP ## make sure libboost=1.73.0
from Bio.PDB.PDBParser import PDBParser

# from Bio.SubsMat.MatrixInfo import blosum62
## if Biopython above 1.76
# from Bio.Align import substitution_matrices
# blosum62 = substitution_matrices.load("BLOSUM62")
## taken from Bio.SubsMat.MatrixInfo import blosum62
blosum62 = {
...
# program to annotate structure - change according to your username
miniconda_path = os.path.dirname(os.getcwd())
DSSP_PATH = f"{miniconda_path}/.conda/envs/forcefields/bin/mkdssp"
...
oxidized_cys_correction = {"H": 0.11, "HA": 0.16, "C": 0, "CA": -2.8, "CB": 13.1,
                           "N": -0.2}

secondary_struc_dict = dict(zip(['H', 'B', 'E', 'G', 'I', 'T', 'S', '-'],
                                [list(np.identity(8)[i]) for i in range(8)]))
max_asa_dict = dict(zip(paper_order,
                        [121.0, 148.0, 187.0, 214.0, 228.0, 97.0, 216.0, 195.0, 230.0,
                         191.0, 203.0, 187.0, 154.0, 214.0, 265.0, 143.0, 163.0, 165.0,
                         264.0, 255.0]))
...
class PDB_SPARTAp_DataReader(BaseDataReader):
    ...
    def __init__(self):
        super(PDB_SPARTAp_DataReader, self).__init__(
            columns=PDB_SPARTAp_DataReader.COLS_.keys())
    ...
    def blosum_nums(self, res_obj):
       ...
       """
        ...
        cap_1to3 = dict(
            (i, j.upper()) for i, j in IUPACData.protein_letters_1to3.items())
        std_blosum62 = dict(
            [(cap_1to3[i[0]], cap_1to3[i[1]]), j] for i, j in blosum62.items()
    ...
    def calc_phi_psi(chain_obj):
        ...
        fin_list = [out_list[i] for i in range(len(out_list)) if
                    (full_res_list[i].get_id()[2] == ' ')]
        return fin_list
    ...
    def find_nearest_atom(nn_tree, catom, rad, inrad=0.5, atom_type='Any', excl=[]):
        ...
        # If none found, return None
            ...
            for idx, dist in enumerate(dists):
                ...
                if dist < d_min and dist > inrad and dist > 0.1 and (
                        atom_type == atom_elem):
                    at = atoms[idx]
                    if at not in excl:
                        d_min = dist
    ...
    def calc_ring_currents(self, res_object_list):
        ...
        if len(res_numbers) > len(set(res_numbers)):
            raise Exception(
                'Not all residue numbers are unique in calc_ring_currents res_object_list')

        # Parameters from shiftX
        atom_names_target = ['C', 'CA', 'CB', 'N', 'HA', 'HA2', 'HA3', 'H', '1H', '1HA',
                             '2HA']
        intensity_factors = {'PHE': 1.05, 'TYR': 0.92, 'TRP1': 1.04, 'TRP2': 0.90,
                             'HIS': 0.43}
        target_factors = {'HA': 5.13, 'HA2': 5.13, 'HA3': 5.13, 'H': 7.06, 'CA': 1.5,
                          'CB': 1.00, 'N': 1.00, 'C': 1.00, '1HA': 5.13, '2HA': 5.13,
                          '1H': 7.06}
        ...
        for res in res_object_list:
            try:
                if (res.get_resname() == 'PHE'):
                    rings += [['PHE', (res['CG'].get_coord(), res['CD2'].get_coord(),
                                       res['CE2'].get_coord(), res['CZ'].get_coord(),
                                       res['CE1'].get_coord(), res['CD1'].get_coord())]]
                elif (res.get_resname() == 'TYR'):
                    rings += [['TYR', (res['CG'].get_coord(), res['CD2'].get_coord(),
                                       res['CE2'].get_coord(), res['CZ'].get_coord(),
                                       res['CE1'].get_coord(), res['CD1'].get_coord())]]
                elif (res.get_resname() == 'TRP'):
                    rings += [['TRP1', (res['CD2'].get_coord(), res['CE3'].get_coord(),
                                        res['CZ3'].get_coord(), res['CH2'].get_coord(),
                                        res['CZ2'].get_coord(),
                                        res['CE2'].get_coord())]]
                    rings += [['TRP2', (res['CG'].get_coord(), res['CD2'].get_coord(),
                                        res['CE2'].get_coord(), res['NE1'].get_coord(),
                                        res['CD1'].get_coord())]]
                elif (res.get_resname() == 'HIS'):
                    rings += [['HIS', (res['CG'].get_coord(), res['ND1'].get_coord(),
        ...
        for res in res_object_list:
            resnum = res.get_id()[1]
            target_atoms = [i for i in res.get_atoms() if
                            i.get_id() in atom_names_target]
            for atom in target_atoms:
                shift = 0
                for ring in rings:
                    G = 0
                    ring_coords = ring[1]
                    normal = np.cross(ring_coords[1] - ring_coords[0],
                                      ring_coords[-1] - ring_coords[0])
                    normal = normal / np.linalg.norm(normal)
                    o = atom.get_coord() + np.dot(normal, ring_coords[
                        0] - atom.get_coord()) * normal
                    for i in range(len(ring_coords)):
                        if (i == (len(ring_coords) - 1)):
                            ...
                            d_ij = 1 / (np.linalg.norm(d_r_i) ** 3) + 1 / (
                                        np.linalg.norm(d_r_j) ** 3)
                        else:
                            r_i = ring_coords[i] - o
            ...
            included_atoms = [i.split('_')[0] for i in
                              res_ring_shift_dict[resnum].keys()]
            ...
            if res_ring_shift_dict[resnum]['H_RING'] is np.nan:
                res_ring_shift_dict[resnum]['H_RING'] = res_ring_shift_dict[resnum][
                    '1H_RING']
            if res_ring_shift_dict[resnum]['HA_RING'] is np.nan:
                res_ring_shift_dict[resnum]['HA_RING'] = res_ring_shift_dict[resnum][
                    '1HA_RING']

        return res_ring_shift_dict

    def NH_O_bond(self, nn_tree, res_obj, im1_atoms, ip1_atoms, rad, atom0, atom1,
                  at_type, efilt=False):
        ...
        while (angle2 < (math.pi / 2)) or (angle1 < (math.pi / 2)):
            ...
            atom2 = self.find_nearest_atom(nn_tree, atom1, rad, atom_type=at_type,
                                           excl=full_excl)
            if atom2 is None:
                return 5 * [
                    0]  # If there are no atoms of the specified type within rad of atom1 and accounting for exclusions, then return list of zeros for HNbond_params
            excl_at += [atom2]
            angle2 = PDB.calc_angle(atom0.get_vector(), atom1.get_vector(),
                                    atom2.get_vector())
            ...
            try:
                atom3 = self.find_nearest_atom(parent_tree, atom2, rad,
                                               atom_type=['N', 'C', 'O'])
                # HNOCatom = HNOatom.get_parent()['C']
                angle1 = PDB.calc_angle(atom1.get_vector(), atom2.get_vector(),
                                        atom3.get_vector())
                energy = 0.084 * 332 * (
                            1 / (atom0 - atom2) + 1 / (atom1 - atom3) - 1 / (
                        bond_dist) - 1 / (atom0 - atom3))
            except AttributeError:
                angle1 = math.pi * 109.5 / 180
                energy = 0
                if angle2 >= math.pi / 2:
                    return [bond_dist, math.cos(angle1), math.cos(angle2), 1,
                            energy]  # Return the parameter list first so that it will not go through energy check
    ...
    def hbond_network(self, nn_tree, res_obj, rad=3 * [5.0], ha_bond='restrictive',
                      efilt=False, efilter_O2=True):
        '''
        ...
        if HAatom is None:
            ...
            if ha_bond == 'permissive':
                HAOatom = self.find_nearest_atom(nn_tree, HAatom, rad[0],
                                                 atom_type=['N', 'O'], excl=res_i_atoms)
                if HAOatom is None:
                    pass
                else:
                    HAangle2 = PDB.calc_angle(ACatom.get_vector(), HAatom.get_vector(),
                                              HAOatom.get_vector())
                    ...
                    if HAOCatom is None:  # This should indicate that the Oxygen in question is in a water molecule
                        ...
                        HAenergy = 0
                        flag = 1
                    HAbond_Params = [HAbond_dist, math.cos(HAangle1),
                                     math.cos(HAangle2), flag, HAenergy]
                ...
                while (HAangle2 < (math.pi / 2)) or (HAangle1 < (math.pi / 2)):
                    ...
                    HAOatom = self.find_nearest_atom(nn_tree, HAatom, rad[0],
                                                     atom_type='O', excl=full_excl)
                    if HAOatom is None:  # No possible HA bond so break loop leaving HAbond_Params as 5*[0]
                        HAbond_Params = [0] * 5
                        break
                    else:
                        excl_at_Ha += [
                            HAOatom]  # Add this acceptor to the exclusion list so that if we redo the loop it will not be included in the search again
                        HAangle2 = PDB.calc_angle(ACatom.get_vector(),
                                                  HAatom.get_vector(),
                                                  HAOatom.get_vector())
                        HAbond_dist = HAatom - HAOatom
                        try:
                        ...
                        if HAOCatom is None:  # This should indicate that the Oxygen in question is in a water molecule so set angle and energy to some default values but keep the HA bond
                            ...
                            HAangle1 = PDB.calc_angle(HAatom.get_vector(),
                                                      HAOatom.get_vector(),
                                                      HAOCatom.get_vector())
                            HAenergy = 0.084 * 332 * (1 / (ACatom - HAOatom) + 1 / (
                                        HAatom - HAOCatom) - 1 / (HAbond_dist) - 1 / (
                                                                  ACatom - HAOCatom))
                            O_idx = all_res.index(
                                HAOatom.get_parent())  # Get index of acceptor Oxygen in list of residues so we can exclude the correct neighbor residue when we check if this Oxygen has a secondary H-bond
                            try:
                                ...
                                O_ip1_atoms = []
                            O_ip1_atoms += [
                                HAatom]  # Need to exclude the H-alpha atom from the neighbor search but can't exclude all atoms on res_obj since the secondary H-bond on the Oxygen could be to the HN on res_obj
                            secondary_O_params = self.NH_O_bond(nn_tree,
                                                                HAOatom.get_parent(),
                                                                [], O_ip1_atoms, rad[2],
                                                                HAOCatom, HAOatom,
                                                                ['H', 'D'],
                                                                efilt=efilter_O2)  # Look for H-bond on acceptor Oxygen
                            if secondary_O_params[
                                -2] == 1:  # If acceptor Oxygen has an H-bond, keep candidate HA H-bond else loop will be repeated
                                flag = 1
                        HAbond_Params = [HAbond_dist, math.cos(HAangle1),
                                         math.cos(HAangle2), flag, HAenergy]
        ...
        if HNatom is None:
            ...
            Natom = res_obj['N']
            HNbond_Params = self.NH_O_bond(nn_tree, res_obj, im1_atoms, ip1_atoms,
                                           rad[1], Natom, HNatom, ['N', 'O'],
                                           efilt=efilt)
        if Oatom is None:
            pass
        else:
            Catom = res_obj['C']
            Obond_Params = self.NH_O_bond(nn_tree, res_obj, im1_atoms, ip1_atoms,
                                          rad[2], Catom, Oatom, ['H', 'D'], efilt=efilt)
    ...
    def df_from_file_1res(self, fpath, hbrad=3 * [5.0], ha_bond='restrictive',
                          efilt=False, efilter_O2=True, s2rad=10.0, hse=True,
                          first_chain_only=False, bfact_mode='all', sequence_columns=0):
        ...
        col_names += [i + j for i in ['CHI1_', 'CHI2_'] for j in
                      ['COS', 'SIN', 'EXISTS']]
        col_names += [i + j for i in ['Ha_', 'HN_', 'O_'] for j in
                      ['d_HA', 'COS_H', 'COS_A', 'EXISTS', 'ENERGY']]
        col_names += ['S2']
        ...
        col_names += ['A_HELIX_SS', 'B_BRIDGE_SS', 'STRAND_SS', '3_10_HELIX_SS',
                      'PI_HELIX_SS', 'TURN_SS', 'BEND_SS', 'NONE_SS']
        col_names += ['REL_ASA', 'ABS_ASA']
        col_names += ['DSSP_PHI', 'DSSP_PSI']
        col_names += ['NH-O1_ENERGY', 'NH-O2_ENERGY', 'O-NH1_ENERGY', 'O-NH2_ENERGY']
        ring_column_names = ['C_RING', 'CA_RING', 'CB_RING', 'N_RING', 'H_RING',
                             'HA_RING', 'HA2_RING', 'HA3_RING']
        col_bfact = ['AVG_B']
        col_names += ring_column_names
        col_names += col_bfact
        if sequence_columns > 0:
            seq_match_cols = ['RESNAME_i-' + str(i) for i in
                              range(sequence_columns, 0, -1)] + ['RESNAME_i+' + str(i)
                                                                 for i in range(1,
                                                                                sequence_columns + 1)]
        ...
        for model in structure:
            ...
            try:
                dssp = DSSP(model, fpath, dssp=DSSP_PATH)
            except:
                dssp = []
            if hse:
            ...
            for chain in model:
                if sequence_columns > 0:
                    res_dict = {res.get_id(): idx for idx, res in
                                enumerate(chain.get_unpacked_list())}
                    list_of_resnames = [res.resname for res in
                                        chain.get_unpacked_list()]
                    list_of_resnames = [
                                           'NONE'] * sequence_columns + list_of_resnames + [
                ...
                poly_residues = [res for poly in polys for res in poly if
                                 res.id[2] == ' ']
                # seq = ''.join([PDB.Polypeptide.three_to_one(i.get_resname()) for i in
                #                poly_residues])
                polypeptide = PDB.Polypeptide.Polypeptide(poly_residues)
                ...
                for l, res in enumerate(poly_residues):
                    ...
                    if resname not in AAlet3:
                        raise Exception(
                            'Something is wrong with peptide loop: ' + fpath)
                    # if PDB.Polypeptide.three_to_one(resname) != seq[l]:
                    #     raise Exception(
                    #         'Something is wrong with peptide loop: ' + fpath)
                    if PDB.Polypeptide.index_to_one(PDB.Polypeptide.three_to_index(resname)) != seq[l]:
                        raise Exception(
                            'Something is wrong with peptide loop: ' + fpath)
                    ...
                    row_data += torsions
                    HBonds = self.hbond_network(nn_tree, res, hbrad, ha_bond=ha_bond,
                    ...
                    try:
                        ...
                        row_data += [dssp_dat[7], dssp_dat[9], dssp_dat[11],
                                     dssp_dat[13]]
                    except KeyError:
                        print('KeyError at ' + str(
                            (chain.id, res.id)) + '.  Skipping this residue')
                        row_data += 16 * [0]
                        continue
                    except TypeError:
                        print('DSSP failed on ' + str(
                            (chain.id, res.id)) + '.  Skipping this residue')
                        row_data += 16 * [0]
                        continue
                    ...
                    if sequence_columns > 0:
                        central_res_idx = res_dict[
                                              res.get_id()] + sequence_columns  # Obtain index of central residue, accounting for the 'NONE' padding of list_of_resnames
                        row_data += list_of_resnames[
                                    central_res_idx - sequence_columns: central_res_idx] + list_of_resnames[
                                                                                           central_res_idx + 1: central_res_idx + 1 + sequence_columns]
                    if resname == "CYS":
                        # Check oxidation state of cystiene (whether there are disulfide bonds)
    ...
    def df_from_file_3res(self, fpath, hbrad=3 * [5.0], ha_bond='restrictive',
                          efilt=False, efilter_O2=True, s2rad=10.0, rcshifts=True,
                          hse=True, first_chain_only=False, bfact_mode='all',
                          sequence_columns=0):
        ...
        chi_names = [i + j for i in ['CHI1_', 'CHI2_'] for j in
                     ['COS', 'SIN', 'EXISTS']]
        hbprev_names = ['O_' + j for j in
                        ['d_HA', 'COS_H', 'COS_A', 'EXISTS', 'ENERGY']]
        hb_names = [i + j for i in ['Ha_', 'HN_', 'O_'] for j in
                    ['d_HA', 'COS_H', 'COS_A', 'EXISTS', 'ENERGY']]
        hbnext_names = ['HN_' + j for j in
                        ['d_HA', 'COS_H', 'COS_A', 'EXISTS', 'ENERGY']]
        hse_names = ['HSE_CA' + i for i in ['_U', '_D', '_Angle']]
        hse_names += ['HSE_CB' + i for i in ['_U', '_D']]
        ss_names = ['A_HELIX_SS', 'B_BRIDGE_SS', 'STRAND_SS', '3_10_HELIX_SS',
                    'PI_HELIX_SS', 'TURN_SS', 'BEND_SS', 'NONE_SS']
        asa_names = ['REL_ASA', 'ABS_ASA']
        dssp_phipsi_names = ['DSSP_PHI', 'DSSP_PSI']
        dssp_hbond_names = ['NH-O1_ENERGY', 'NH-O2_ENERGY', 'O-NH1_ENERGY',
                            'O-NH2_ENERGY']
        ...
        col_phipsi = [i + j + k for k in ['i-1', 'i', 'i+1'] for i in ['PHI_', 'PSI_']
                      for j in ['COS_', 'SIN_']]
        # col_phipsi += [i+j for i in ['PHI_', 'PSI_'] for j in ['COS_i', 'SIN_i']]
        # col_phipsi += [i+j for i in ['PHI_', 'PSI_'] for j in ['COS_i+1', 'SIN_i+1']]
        col_chi = [i + j + k for k in ['_i-1', '_i', '_i+1'] for i in ['CHI1_', 'CHI2_']
                   for j in ['COS', 'SIN', 'EXISTS']]
        col_hbprev = ['O_' + i + '_i-1' for i in
                      ['d_HA', '_COS_H', '_COS_A', '_EXISTS', '_ENERGY']]
        col_hbond = [i + j + '_i' for i in ['Ha_', 'HN_', 'O_'] for j in
                     ['d_HA', '_COS_H', '_COS_A', '_EXISTS', '_ENERGY']]
        col_hbnext = ['HN_' + i + '_i+1' for i in
                      ['d_HA', '_COS_H', '_COS_A', '_EXISTS', '_ENERGY']]
        col_s2 = ['S2' + i for i in ['_i-1', '_i', '_i+1']]
        blosum_names = [
            'BLOSUM62_NUM_' + sorted(list(IUPACData.protein_letters_3to1.keys()))[
                i].upper() for i in range(20)]
        col_blosum = [blosum_names[i] + j for j in ['_i-1', '_i', '_i+1'] for i in
                      range(20)]
        ...
        if rcshifts:
            ...
            col_names += col_rccs
        if hse:
            col_hse = [hse_names[i] + j for j in ['_i-1', '_i', '_i+1'] for i in
                       range(5)]
            col_names += col_hse
        ...
        df_1res_all_chains = self.df_from_file_1res(fpath, hbrad, ha_bond=ha_bond,
                                                    efilt=efilt, efilter_O2=efilter_O2,
                                                    s2rad=s2rad, hse=hse,
                                                    first_chain_only=first_chain_only,
                                                    bfact_mode=bfact_mode,
        ...
        for chain in list(set(df_1res_all_chains['CHAIN'].tolist())):
            ...
            for i in range(len(df_1res)):
                ...
                if (res_i_num - 1) not in res_list:
                    ...
                    psi_prev = [df_1res.loc[i - 1, 'PSI_COS'],
                                df_1res.loc[i - 1, 'PSI_SIN']]
                    phi_prev = [df_1res.loc[i - 1, 'PHI_COS'],
                                df_1res.loc[i - 1, 'PHI_SIN']]
                ...
                if (res_i_num + 1) not in res_list:
                    ...
                    psi_next = [df_1res.loc[i + 1, 'PSI_COS'],
                                df_1res.loc[i + 1, 'PSI_SIN']]
                    phi_next = [df_1res.loc[i + 1, 'PHI_COS'],
                                df_1res.loc[i + 1, 'PHI_SIN']]
                ...
                df.loc[i, col_phipsi] = phi_prev + psi_prev + list(
                    df_1res.loc[i, phipsi_names].values) + phi_next + psi_next
                df.loc[i, col_chi] = chi_prev + list(
                    df_1res.loc[i, chi_names].values) + chi_next
                ...
                df.loc[i, col_blosum] = blosum_prev + list(
                    df_1res.loc[i, blosum_names].values) + blosum_next
                if rcshifts: ### from residue get the randomcoil
                    resname = df.loc[i, 'RESNAME']
                    if res_next == 'PRO':
                        rccs = [randcoil_pro[j][resname] for j in atom_names]
                    else:
                        rccs = [randcoil_ala[j][resname] for j in atom_names]
                    if df_1res.loc[i, 'CYS_OX']: #TODO: adding cys_ox
                        if res_next == 'PRO':
                ...
                if hse:
                    df.loc[i, col_hse] = hse_prev + list(
                        df_1res.loc[i, hse_names].values) + hse_next
                df.loc[i, ring_column_names] = list(
                    df_1res.loc[i, ring_column_names].values)
                df.loc[i, col_dssp] = dssp_prev + list(
                    df_1res.loc[i, dssp_names]) + dssp_next
                df.loc[i, bfact_names] = bfact_prev + [
                    df_1res.loc[i, 'AVG_B']] + bfact_next
                if sequence_columns > 0:
    ...
    def df_from_file_tripeptide(self, fpath, hbrad=5.0, s2rad=10.0, rcshifts=True):
        ...
        col_chi = [i + j + k for k in ['_i-1', '_i', '_i+1'] for i in ['CHI1_', 'CHI2_']
                   for j in ['COS', 'SIN', 'EXISTS']]
        col_hbond = [i + j + k for k in ['_i-1', '_i', '_i+1'] for i in
                     ['Ha_', 'HN_', 'O_']
        ...
        for i in range(len(df_1res)):
            ...
            if i == 0:
                ...
                blosum_prev = list(df_1res.loc[i - 1, blosum_names].values)
                phipsi_prev = [df_1res.loc[i - 1, 'PHI_COS'],
                               df_1res.loc[i - 1, 'PHI_SIN'],
                               df_1res.loc[i - 1, 'PSI_COS'],
                               df_1res.loc[i - 1, 'PSI_SIN']]
                chi_prev = list(df_1res.loc[i - 1, chi_names].values)
            ...
            if i == len(df_1res) - 1:
                ...
                blosum_next = list(df_1res.loc[i + 1, blosum_names].values)
                phipsi_next = [df_1res.loc[i + 1, 'PHI_COS'],
                               df_1res.loc[i + 1, 'PHI_SIN'],
                               df_1res.loc[i + 1, 'PSI_COS'],
                               df_1res.loc[i + 1, 'PSI_SIN']]
                chi_next = list(df_1res.loc[i + 1, chi_names].values)
            ...
            # Insert row into DataFrame
            df.loc[i, col_phipsi] = phipsi_prev + list(
                df_1res.loc[i, phipsi_names].values) + phipsi_next
            df.loc[i, col_chi] = chi_prev + list(
                df_1res.loc[i, chi_names].values) + chi_next
            #            df.loc[i, col_hbprev] = hb_prev
            df.loc[i, col_hbond] = hb_prev + list(
                df_1res.loc[i, hb_names].values) + hb_next
            #            df.loc[i, col_hbnext] = hb_next
            df.loc[i, col_s2] = [s2_prev, df_1res.loc[i, 'S2'], s2_next]
            df.loc[i, col_blosum] = blosum_prev + list(
                df_1res.loc[i, blosum_names].values) + blosum_next
...
if __name__ == "__main__":
    sample_pdb = "../reduce_v2/10002_1VEX/af2_00.pdb"
    data_processor = PDB_SPARTAp_DataReader()
    df = data_processor.df_from_file_3res(sample_pdb)
    print(df)
